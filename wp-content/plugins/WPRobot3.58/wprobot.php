...
$wpr_modules = array("amazon","article","clickbank","ebay","flickr","yahooanswers","yahoonews","youtube","rss","translation","twitter","commissionjunction","oodle","pressrelease","shopzilla","itunes","linkshare","eventful","yelp","shareasale","avantlink","plr");
...
function wpr_post($camp_id, $keyword="", $retry=1, $manual=0) {
	...
	// REWRITE
	if($options['wpr_rewrite_active'] == "both") {
		$chox = rand(0,2);
		if($chox == 0) {$options['wpr_rewrite_active'] = "Yes";} elseif($chox == 1) {$options['wpr_rewrite_active'] = "sc";} else {$options['wpr_rewrite_active'] = "tbs";}
	}	
	...
	if($options['wpr_rewrite_active'] == "Yes" && $yahoocat["rw"] == 1) {
		...
		if(empty($options['wpr_sc_rewrite_pw']) || empty($options['wpr_sc_rewrite_email'])) {
			...
			$rewrite = wpr_sc_rewrite($content,$options['wpr_sc_rewrite_email'],$options['wpr_sc_rewrite_pw'],$options['wpr_sc_quality'],$keyword,$options['wpr_rewrite_protected'],$options['wpr_sc_port']);
			...
			// REWRITE TITLE
			if($options['wpr_tbs_rewrite_title'] == "Yes") {
				$rewritetitle = wpr_sc_rewrite($title,$options['wpr_sc_rewrite_email'],$options['wpr_sc_rewrite_pw'],$options['wpr_sc_quality'],$keyword,$options['wpr_rewrite_protected'],$options['wpr_sc_port']);
				if(!empty($rewritetitle["error"]["reason"])) {
					$esql = "INSERT INTO ".$wpr_table_errors." ( campaign, keyword, module, reason, message, time ) VALUES";
	...
	if (is_wp_error($insert)) {
		...
		if($yahoocat["t"] == 1 && !empty($ximage)) {
			...
			if(isset($post_thumbnail)) {
				...
				add_post_meta($post_thumbnail, "_wp_attachment_metadata", serialize(array("file" => $zimage)));
...
[file] => 2011/01/lola3-5-1500-001.jpg
[sizes] => Array
(
	[thumbnail] => Array
	(
	[file] => lola3-5-1500-001-150x150.jpg
	[width] => 150
	[height] => 150
	)
