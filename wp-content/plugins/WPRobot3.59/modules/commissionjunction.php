		...
		if (isset($pxml->products->product)) {
			foreach($pxml->products->product as $item) {		
				...
				if ($skipit == 0) {
					...
					if(!empty($item->upc) && $item->upc != "") {$unique = $item->upc;echo "CHECK1<br>";} elseif(!empty($item->sku)) {$unique = $item->sku;echo "CHECK2<br>";} elseif(!empty($item->isbn)) {$unique = $item->isbn;echo "CHECK3<br>";}
