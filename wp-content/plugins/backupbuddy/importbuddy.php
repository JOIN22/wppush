 ...
 *
 * ITERATION: 114 - 6/21/2010
 * 
 * http://pluginbuddy.com
 *
 */
$_ver = '1.1.36'; // Last edit to importbuddy made on this version. Do not edit.
...
//
//
// Contact support if you need assistance with BackupBuddy or importbuddy.php.
//
// THERE ARE NO LONGER ANY USER CONFIGURABLE SETTINGS IN THIS FILE.
// ALL ADVANCED DEBUGGING FEATURES CAN BE ACCESSED IN STEP 1!
//
//
...
// ALL DEBUG OPTIONS ARE NOW AVAILABLE DURING STEP 1 OF THE SCRIPT. DO NOT EDIT THESE HERE!
$pass = '#PASSWORD#'; // Password for usage. Do not include pound signs.
$skip_files = false; // Skip file extraction process - Default = false.
$skip_database = false; // Skip database import and migration - Default = false;
$skip_htaccess = false; // Skip migration of htaccess file - Default = false;
$force_compatibility = false; // SET true TO FORCE USE OF PHP-BASED UNZIPPING IF AUTOMATIC COMPATIBILITY MODE NEEDS TO BE FORCED ON.
$enhanced_debug_reporting = false; // SET true TO ENABLE ENHANCED PHP ERROR REPORTING FOR ENHANCED DEBUGGING.

if ( $_POST['skip_files'] == 'on' ) { $skip_files = true; }
if ( $_POST['skip_database'] == 'on' ) { $skip_database = true; }
if ( $_POST['skip_htaccess'] == 'on' ) { $skip_htaccess = true; }
if ( $_POST['force_compatibility'] == 'on' ) { $force_compatibility = true; }
if ( $_POST['enhanced_debug_reporting'] == 'on' ) { $enhanced_debug_reporting = true; }



if ( $enhanced_debug_reporting == true ) {
	error_reporting(E_ERROR | E_WARNING | E_PARSE | E_NOTICE); // HIGH
} else {
	error_reporting(E_ALL ^ E_NOTICE); // LOW
}
// 3600=30min time limits
...
if (isset($_GET['ezimg'])) ezimg::showImg($_GET['ezimg']); // Return image.
...
switch ($step) {
	case 1:
		echo 'Please select the backup file you would like to import.<br />';
		echo 'Begin with a Full Backup then import the latest database backup if one exists.<br />';
		echo 'Import settings for migrating may be changed on Step 4.<br /><br />';
		echo 'Throughout the restore process you may hover over <a class="ithemes_tip" title=" - This is an example help tip. Hover over these for additional help.">(?)</a> for additional help.<br />';
		echo 'For additional help visit <a href="http://pluginbuddy.com/tutorial-backupbuddy/">BackupBuddy Tutorials</a> or <a href="http://pluginbuddy.com/support/">Official Support</a>.';
		echo '<br />';
		echo '<br /><br /><form action="?step=2" method=post>';
		if ($handle = opendir( dirname( __FILE__ ) )) {
			...
			if ($count == 0) {
				...
				echo 'ERROR: Unable to find any BackupBuddy ZIP files. Upload the BackupBuddy ZIP file into the same directory as this file and keep the original file name.';
			}
			closedir($handle);
		} else {
		...
		if ($count > 0) {
		?>
		<span class="toggle" id="advanced">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&middot; Display advanced troubleshooting options ...</span>
		<div id="toggle-advanced" class="toggled">
			<b>WARNING: Advanced users only unless directed by support.</b><br /><br />
			<input type="checkbox" name="skip_files" /> Skip zip file extraction. <br />
			...
			<input type="checkbox" name="force_compatibility" /> Force unzipping compatibility mode. <br />
			<input type="checkbox" name="enhanced_debug_reporting" /> Enhanced error & warning messages. <br />
		</div>
		<br /><br /><br /><br />
		<input type="submit" name="submit" value="Next Step &raquo;" class="button" />
		<?php } ?>
		</form>
		<?php 
		break;
	case 2:
		//TODO: Do some checks here, such as existing files.
		/*
		echo '<b>Verify Directory</b><br /><br />';
		echo 'Verify that this script is in the directory you would like the site migrated or restored into. Conflicts may occur if an existing WordPress installation exists.<br /><br />';
		echo 'Server Path: ' . ABSPATH;
		echo '<br ><br />';
		*/

		//echo '<b>Import Type</b><br /><br />';
		echo 'Please select whether you are migrating to a new server with new database settings OR restoring the site to the same server the backup was made on. This will help the script determine the best defaults for you.';
		echo '<br />';
		...
		echo '<input type="hidden" name="skip_files" value="' . $_POST['skip_files'] . '" />';
		echo '<input type="hidden" name="skip_database" value="' . $_POST['skip_database'] . '" />';
		echo '<input type="hidden" name="skip_htaccess" value="' . $_POST['skip_htaccess'] . '" />';
		echo '<input type="hidden" name="force_compatibility" value="' . $_POST['force_compatibility'] . '" />';
		echo '<input type="hidden" name="enhanced_debug_reporting" value="' . $_POST['enhanced_debug_reporting'] . '" />';
		
		echo '<p><label for="type_migrate" style="width: 170px;">Migrate to new server:</label><input type="radio" name="type" id="type_migrate" value="migrate" style="margin-top: 6px;" checked /></p>';
		echo '<p><label for="type_restore" style="width: 170px;">Restore to same server:</label><input type="radio" name="type" id="type_restore" value="restore" style="margin-top: 6px;" /></p>';
		echo '<input type="hidden" name="file" value="'.htmlentities($_POST['file']).'" />';

		echo '<br />Your site will be imported to the following path on the next step. Conflicts may occur if an existing WordPress installation exists. Path: ' . ABSPATH;
		if ( $existing_wordpress === true ) {
			echo '<br /><br />';
	...
	case 3:
			...
			if ( ( false === $skip_files ) && ( false === $failed ) ) { // Option to skip all file updating / extracting.
				//restore_files(); // If www folder is missing, continue since it may be db only restore. Handle this better in next ver.
				echo 'Extracting files into ' . ABSPATH . '<br />';
				$failed = !bb_unzip( ABSPATH . '/' . $_POST['file'], ABSPATH, $options );
				...
				$zip_id = substr( $_POST['file'], $zip_id, strlen($_POST['file'])-$zip_id-4 );
				$backupdata_file = ABSPATH . '/wp-content/uploads/temp_'.$zip_id.'/backupbuddy_dat.php'; // Full backup dat file location
				if ( ( !file_exists( $backupdata_file ) ) && ( !file_exists( ABSPATH . '/backupbuddy_dat.php' ) ) ) {
					$failed = true;
					echo 'ERROR! The unzip process reported success but the backup data file, backupbuddy_dat.php was not found in the extracted files. The unzip process either failed or the zip file is not a proper BackupBuddy backup.';
				}
			}
			
			if ( false === $failed ) {
				echo '<br /><b>Files extracted.</b> <a class="ithemes_tip" title=" - Your site files and/or database have been extracted from the ZIP backup file for further processing during the next steps.">(?)</a>';
				echo '<form action="?step=4" method=post>';
				echo '<input type="hidden" name="skip_files" value="' . $_POST['skip_files'] . '" />';
				echo '<input type="hidden" name="skip_database" value="' . $_POST['skip_database'] . '" />';
				echo '<input type="hidden" name="skip_htaccess" value="' . $_POST['skip_htaccess'] . '" />';
				echo '<input type="hidden" name="file" value="'.htmlentities($_POST['file']).'" />';
				echo '<input type="hidden" name="type" value="'.htmlentities($_POST['type']).'" />';
				echo '<input type="hidden" name="force_compatibility" value="' . $_POST['force_compatibility'] . '" />';
				echo '<input type="hidden" name="enhanced_debug_reporting" value="' . $_POST['enhanced_debug_reporting'] . '" />';
				echo '<br /><br /><input type="submit" name="submit" value="Next Step &raquo;" class="button" />';
				echo '</form>';
			} else {
				// TODO: Better failure handling message
				echo '<br /><br />File extraction process did not complete successfully. Unable to continue to next step.<br /><br />';
	...
	case 4:
		...
		if (true) {
			...
			if ( $_POST['type'] == 'migrate' ) {
				...
				$backupdata['db_server'] = 'localhost'; // We dont prefill restore database stuff!
				$backupdata['db_name'] = '';
				$backupdata['db_user'] = '';
				$backupdata['db_password'] = '';
			} else { // restore...
				echo 'Please enter the settings for restoring to your same server using the same database.  Your previous settings have been entered for you for your convenience.<br /><br />';
				echo 'IMPORTANT: Two WordPress installations CANNOT share the same database settings at the same time.  You must create a new database or use a different prefix if you will be using two different WordPress installs simultaneously.<br /><br /><br />';
			}
			
			
			//echo 'Please enter the settings for the NEW destination site. If you are migrating to a new host you will need to change your database information:<br /><br /><br />';
			...
			echo '<b>General Settings:</b><br /><br />';
			echo '<label>New Blog URL <a class="ithemes_tip" title=" - This is the site address where you want the final site you are restoring / migrating to reside.">(?)</a>:</label> <input type="text" name="siteurl" value="'. $url .'" size="50" /><br />';
			
			if ($url != $backupdata['blogname']) {
				//echo '<center><i>Note: A new site URL was detected and automatically entered for you.</i></center><br />';
			}
			echo '<label>Blog Name <a class="ithemes_tip" title=" - This is the name of your blog.">(?)</a>:</label> <input type="text" name="blogname" value="'.$backupdata['blogname'].'" size="50" /><br />';
			echo '<label>Blog Description <a class="ithemes_tip" title=" - This is the description of your blog.">(?)</a>:</label> <input type="text" name="blogdescription" value="'.$backupdata['blogdescription'].'" size="50" /><br /><br /><br />';
			
			echo '<b>Database Settings (must be unique per WordPress install) <a class="ithemes_tip" title=" - These settings control where your backed up database will be restored to.  If you are restoring to the same server, the settings below will import the database to your existing WordPress database location, overwriting your existing WordPress database already on the server.  If you are moving to a new host you will need to create a database to import into. The database settings MUST be unique for each WordPress installation.  If you use the same settings for multiple WordPress installations then all blog content and settings will be shared, causing conflicts!">(?)</a>:</b><br /><br />';
			echo '<label>Database Server <a class="ithemes_tip" title=" - This is the address to the mySQL server where your database will be stored.  99% of the time this is localhost.  The location of your mySQL server will be provided to you by your host if it differs.">(?)</a>:</label> <input type="text" name="db_server" id="mysql_server" value="'.$backupdata['db_server'].'" size="50" /><br />';
			echo '<label>Database Name <a class="ithemes_tip" title=" - This is the name of the database you want to import your blog into. The database user must have permissions to be able to access this database.  If you are migrating this blog to a new host you will need to create this database (ie using CPanel or phpmyadmin) and create a mysql database user with permissions.">(?)</a>:</label> <input type="text" name="db_name" id="mysql_name" value="'.$backupdata['db_name'].'" size="50" /><br />';
			echo '<label>Database User <a class="ithemes_tip" title=" - This is the database user account that has permission to access the database name in the input above.  This user must be given permission to this database for the import to work.">(?)</a>:</label> <input type="text" name="db_user" id="mysql_user" value="'.$backupdata['db_user'].'" size="50" /><br />';
			echo '<label>Database Pass <a class="ithemes_tip" title=" - This is the password for the database user.">(?)</a>:</label> <input type="text" name="db_password" id="mysql_password" value="'.$backupdata['db_password'].'" size="50" /><br />';
			echo '<label>Database Prefix <a class="ithemes_tip" title=" - This is the prefix given to all tables in the database.  If you are cloning the site on the same server AND the same database name then you will want to change this or else the imported database will overwrite the existing tables.">(?)</a>:</label> <input type="text" name="db_prefix" id="mysql_prefix" id="mysql_prefix" value="'.$backupdata['db_prefix'].'" size="50" />';
			echo '<br />';
			...
			echo '<label>&nbsp;</label> <input type="button" style="" id="ithemes_mysql_test" value="Test database login ..." />
				<span style="display: none;" id="ithemes_loading">';
			...
			echo 'ERROR: Unable to read zip file or not a valid BackupBuddy backup.';
		}
		break;
	case 5:
			$failed = false;

			if ( ( false === $skip_htaccess ) && ( file_exists( ABSPATH . '/.htaccess' ) ) ) {
				// MIGRATE HTACCESS
				$backupdata=_backup_data();

				$oldurl = strtolower( $backupdata['siteurl'] );
				...
				$newurl = strtolower( $_POST['siteurl'] );
				...
				if ( $newurl !== $oldurl ) {
					// URL subdirectory differs so we need to update htaccess if it exists...
					echo 'URL subdirectory is different. Updating permalink paths in .htaccess ... ';
			...
			if ( ( false === $skip_database ) && ( false === $failed ) ) { // Option to skip all database modifying and checking.
			
				$backupdata = _backup_data();
				
				//echo "<div>First</div>\n";
				$failed = restore_database( $zip, $backupdata );
			}
			if ( false === $failed ) {
				//echo "<div>Third</div>\n";
				$failed = migrate_wp_config();
			}
			if ( false === $failed ) {
			
				// CLEANUP
				//$zip_id = strrpos($_POST['file'],'_')+1;
				...
				$zip_id = substr( $_POST['file'], $zip_id, strlen($_POST['file'])-$zip_id-4 );
				if ( file_exists( ABSPATH . '/wp-content/uploads/temp_'.$zip_id.'/backupbuddy_dat.php' ) ) { unlink( ABSPATH . '/wp-content/uploads/temp_'.$zip_id.'/backupbuddy_dat.php' ); }
				if ( file_exists( ABSPATH . '/wp-content/uploads/temp_'.$zip_id.'/db.sql' ) ) { unlink( ABSPATH . '/wp-content/uploads/temp_'.$zip_id.'/db.sql' ); }
				if ( file_exists( ABSPATH . '/backupbuddy_dat.php' ) ) { unlink( ABSPATH . '/backupbuddy_dat.php' ); }
				if ( file_exists( ABSPATH . '/db.sql' ) ) { unlink( ABSPATH . '/db.sql' ); }
			
				echo '<br /><br />';
				echo '<b>Import / migration Complete!</b><br /><br />Please verify your new site functions properly before deleting importbuddy.php and the ZIP backup.';
			}
		break;
	default:
...
/* bb_unzip()
 *
 * Unzips a BackupBuddy zip file into the given destination directory.
 * Handles compatibility with older PHP versions and fallback for lack of ZIP command line.
 *
 * @param	$file		ZIP filename, including full path.
 * @param	$directory	Path to directory to extract into.
 * @param	$options	(optional) Options & configuration.
 *
 */
function bb_unzip( $file, $directory, $options ) {
	if ( class_exists( 'ZipArchive' ) && ( true === ( $result = _extract_native_zip( $file, $directory, $options ) ) ) ) {
		// Unzip success!
		echo 'Using high speed command line ZIP extraction.<br />';
		return true;
	} else {
		if ( ! class_exists( 'ZipArchive' ) ) {
			echo "<p>ZipArchive class not available ";
			
			if ( version_compare( phpversion(), '5.0.0', '<' ) )
				echo "as this server runs PHP 4. See if your host can upgrade to PHP 5.2+.</p>\n";
			else if ( version_compare( phpversion(), '5.2.0', '<' ) )
				echo "as this server runs PHP " . phpversion() . ". See if your host can upgrade to PHP 5.2+.</p>\n";
			else
				echo "as this server's PHP wasn't compiled with the --enable-zip. See if your host is able to update PHP with this option.</p>\n";
		}
		else
			echo "<p>Native zip function unavailable: {$result['error']}</p>\n";
		
		echo "<p>Falling back to slower compatibility method. Extracting ...</p>\n";
		
		$result = _extract_zip_compat( $file, $directory, $options );
		return $result;
	}
}
...
function _backup_data() {
	...
	$backupdata_file = ABSPATH . '/wp-content/uploads/temp_'.$zip_id.'/backupbuddy_dat.php'; // Full backup dat file location
	if ( file_exists( $backupdata_file ) ) {
		$backupdata = file_get_contents( $backupdata_file );
	} elseif ( file_exists( ABSPATH . '/backupbuddy_dat.php' ) ) {
		$backupdata = file_get_contents( ABSPATH . '/backupbuddy_dat.php' ); // DB only dat file location
	} else {
		echo 'ERROR #59233894:<br /><br />';
		echo 'NOTE: Did you refresh after finishing the import? If so, seeing this is normal!<br /><br />';
		echo 'BackupBuddy data file missing or unreadable. There may be a problem with the backup file, the files could not be extracted (you may manually extract the zip file in this directory to manually do this portion of restore), or the files were deleted before this portion of the restore was reached.  Start the import process over or try manually extracting (unzipping) the files then starting over. Restore will not continue to protect integrity of any existing data.';
		die();
	}
	return unserialize($backupdata);
}



function _extract_native_zip( $file, $directory, $options ) {
	...
	if ( strpos( $phpinfo['PHP Core']['disable_functions'], 'exec') != true ) { // If exec is not explicitly disabled in PHP...
		...
		//echo ABSPATH . '\unzip.exe';
		if ( file_exists( ABSPATH . '\unzip.exe' ) ) {
			echo '<br />Attempting to use provided unzip.exe for native Windows zip functionality.<br /><br />';
		...
		echo '<br /><br />Exec() function is explicitly disabled in PHP, forcing compatibility mode.<br />';
		$exec_return_b = '-1';
	}
	
	global $force_compatibility;
	...
	if ( ( !file_exists ( ABSPATH . '/wp-content/uploads/temp_'.$zip_id.'/backupbuddy_dat.php' ) ) && ( !file_exists ( ABSPATH . '/backupbuddy_dat.php' ) ) ) {
		//echo 'failed native';
		$exec_return_b = '-1';
	}
	...
	if ( ( ! file_exists( $file ) ) || ( $exec_return_b == '-1' ) ) {
		return array( 'error' => 'Backup file couldn\'t be imported without entering slower compatibility mode. <a class="ithemes_tip" title=" - Due to the unavailability of the PHP exec() function and/or native ZIP function, the script has fallen back to a slower, less efficient method.  While slower, this should allow you to continue the import / migration process on this server.  As long as the import process succeeded you should not need to worry about this.">(?)</a>' );
	}
...
function _extract_zip_compat( $file, $directory, $options ) {
	$archive = new PclZip( $file );
	
	//echo $directory;
	//$arguments = array( PCLZIP_OPT_PATH, $directory );
	//$arguments = array( '/', $directory );


	if ( file_exists( $file ) )
		//$result = call_user_func_array( array( &$archive, 'extract' ), $arguments );
		...
		echo 'Unable to read zip file. File not found? Error #54556565834.';
...
function _remap_options( $options ) {
			$translate_options = array(
				...
				'no_compress'				=> PCLZIP_OPT_NO_COMPRESSION
			);
...
complete();if(settings(this).fixPNG)helper.parent.unfixPNG();}})(jQuery);
		...
		<body>
			...
			<?php
			global $skip_files, $skip_database, $skip_htaccess, $force_compatibility, $enhanced_debug_reporting;
			if ( $skip_files != false ) {
			...
			if ( $skip_htaccess != false ) {
				echo 'WARNING: Debug option to skip migrating the htaccess file is set to true. The file will not be migrated if needed.<br />';
			}
			if ( $force_compatibility != false ) {
				echo 'WARNING: Debug option to force compatibility mode. This may result in slower, less reliable operation.<br />';
			}
			if ( $enhanced_debug_reporting != false ) {
			...
			<div style="max-width: 600px; margin:10px; padding: 20px; border:1px solid #ccc; -moz-border-radius: 10px; -webkit-border-radius: 10px; background: #F9F9F9; -moz-box-shadow: 10px 10px 15px -12px #356D8F; -webkit-box-shadow: 10px 10px 15px -12px #356D8F; padding-right: 20px;">
				<div class="wrap">
					<!-- div class="menu" -->
						<h2><?php echo 'Step '.$step.' of '.$step_total; ?></h2>
					<!-- /div -->
					<div class="innerwrap">
		<?php
	}
	function print_footer() {
		...
		echo '</div>';
		echo '</div></div>';
		echo '<div style="clear: both;"><br /><br /><a href="http://pluginbuddy.com" style="text-decoration: none; vertical-align: -2px;">'.ezimg::genImageTag('pluginbuddy.png').' PluginBuddy.com</a></div>';
		global $_ver; // Need to access this here for printing since we arent using a class yet.
		if ( $_ver == '#VERSION#') {
			//echo '<i>Version Unknown</i>';
		} else {
			echo '<br /><i>This script last updated at BackupBuddy v' . $_ver . '</i>';
		}
		echo '</body>';
		echo '</html>';
}
...
function extract_www($zipfile, $path) {
	if (file_exists($zipfile)) {
		...
		if ($zip->open($zipfile) === TRUE) {
			for($i = 0; $i < $zip->numFiles; $i++) {
				...
				if (strpos($entry, "www/") === 0) { // Only extract contents of www/
					//echo 'Entry:'.$entry.'<br />';
					$files[] = $entry; //Add the entry to our array if it in in our desired directory
				}
			}
			...
			if ($zip->extractTo($path, $files) === TRUE) {
				...
				echo '<br />Warning: Unable to extract web files. This is normal for database only backups.<br />';
				$zip->close();
				return FALSE;
			}
		} else {
		...
		echo 'Missing ZIP file for import.';
		return FALSE;
	}
}
...
function restore_database( $zip, $backupdata ) {
	// CREATE DATABASE CONNECTION.
	if ( false === mysql_connect( $_POST['db_server'], $_POST['db_user'], $_POST['db_password'] ) ) {
		echo '<div>' . ezimg::genImageTag( 'bullet_error.png' ) . ' ERROR: Unable to connect to database properly. Verify the database server name, user, password, and database name.</div>';
		return false;
		
		// TODO: This should return the user to step 3 with the error message.
	}
	
	
	
	$database_name = mysql_real_escape_string( $_POST['db_name'] );
	mysql_query( "CREATE DATABASE IF NOT EXISTS $database_name" );
	
	if ( mysql_error() ) {
		echo 'Database already exists. Using existing.<br />';
	} else {
		echo 'Creating database ... Done.<br />';
	}
	flush();
	
	
	if ( false === @mysql_select_db( $_POST['db_name'] ) ) {
		echo ezimg::genImageTag( 'bullet_error.png' )."ERROR: Unable to connect to database. Check your login and other database settings.<br />\n" . mysql_error() . "<br />\n";
		return false;
	}
	...
	// CHECK EXISTANCE OF EXISTING TABLES WITH PREFIX.
	if ( $_POST['db_prefix'] == '' ) {
		echo 'ERROR: A database prefix is required.';
	}
	$prefix = mysql_real_escape_string( $_POST['db_prefix'] );
	$result = mysql_query( "SHOW TABLES LIKE '$prefix%'" );
	
	if ( mysql_num_rows( $result ) > 0 ) {
		$found_existing = true;
		$existing_count = mysql_num_rows( $result );
	}
	unset( $result );
	
	
	// IMPORT SQL FILE
	if ( true ===  do_db_import( $zip, $backupdata ) ) {
		echo ' Done.<br />';
		
		if ( $found_existing ) {
			echo ezimg::genImageTag('bullet_error.png').' Found '.$existing_count.' existing tables with same database prefix found and merged.<br />';
		}
	} else {
		if ( $found_existing ) {
			echo '<br />';
			echo ezimg::genImageTag('bullet_error.png').' Found '.$existing_count.' existing tables with same database prefix found and merged. This may cause warnings.<br />';
		}
	}
	$found_existing = false;
	unset($existing_count);
	
	
	echo 'Migrating SQL content ...';
	
	$search_for   = $backupdata['siteurl'];  // the value you want to search for
	$replace_with = preg_replace( '|/*$|', '', $_POST['siteurl'] );  // the value to replace it with
	
	// Loop through the tables.
	$tables = mysql_list_tables($_POST['db_name']);
	//while ($table_rows = mysql_fetch_array($tables_list)) {
		//$table = $table_rows['Tables_in_'.$db];
	while ($td = mysql_fetch_array($tables)) {
		$table = $td[0];
		
		// TODO: $db_prefix is never set... this needs checked.
		if (substr($table, 0, strlen($db_prefix)) == $db_prefix) {
			...
			// Gt the data and do search and replaces on it...
			$SQL = "SELECT * FROM ".$table;     // Fetch the table contents.
			$data = mysql_query($SQL);
			if (!$data) { echo("ERROR: " . mysql_error() . "<br/>$SQL<br/>"); } 
	...
	// UPDATE SQL DATA WHERE NEEDED:
	
	$old_prefix = $backupdata['db_prefix'];
	$new_prefix = mysql_real_escape_string( $_POST['db_prefix'] );
	...
	$old_blogname = $backupdata['blogname'];
	$new_blogname = mysql_real_escape_string( $_POST['blogname'] );
	if ($old_blogname != $new_blogname ) {
		$SQL = "UPDATE ".$new_prefix."options SET option_value = '" . $new_blogname . "' WHERE option_name='blogname' LIMIT 1;";
		mysql_query($SQL);
		echo 'Updated blog name.';
	}
	
	$old_blogdescription = $backupdata['blogdescription'];
	$new_blogdescription = mysql_real_escape_string( $_POST['blogdescription'] );
	if ($old_blogdescription != $new_blogdescription ) {
		$SQL = "UPDATE ".$new_prefix."options SET option_value = '" . $new_blogdescription . "' WHERE option_name='blogdescription' LIMIT 1;";
		mysql_query($SQL);
		echo 'Updated blog description.';
	}
	
	
	echo ' Checked '.$count_tables_checked." tables, ".$count_items_checked." items. ".$count_items_changed." updated. Done.<br />";
...
function restore_files() {
	echo 'Restoring files to temporary location. This may take several minutes... ';
	flush();
	...
	//remove_dir( ABSPATH . '/www/' ); // Prep for unzipping.
	//$temp_www_dir = tempnam(ABSPATH, "www");
	$temp_www_dir = ABSPATH . '/www_'.rand_string(10);
	
	//echo $temp_www_dir;
	if ( !is_dir( $temp_www_dir ) ) {
		mkdir( $temp_www_dir );
	}
	
	if ( extract_www( ABSPATH . '/' . $_POST['file'], $temp_www_dir ) ) { // Unzip.
		echo 'Done.<br />';
		echo 'Moving files to final location ... ';
		
		$move_char_count = 0;
		$handler = opendir( $temp_www_dir . '/www');
		
		while ( $file = readdir( $handler ) ) {
			if ( $file != '.' && $file != '..') {
				if ( file_exists( ABSPATH . '/' . $file ) ) { // Delete destination if it already exists.
					if ( is_dir( ABSPATH . '/' . $file ) ) { // Is directory?
						remove_dir( ABSPATH . '/' . $file );
					} else { // File.
						unlink( ABSPATH . '/' . $file );
					}
				}
				rename( $temp_www_dir . '/www/' . $file, ABSPATH . '/' . $file );
				echo '.';
				$move_char_count++;
				if ( $move_char_count >= 60 ) {
					echo '<br />';
					$move_char_count=0;
				}
			}
		...
		// remove_dir( ABSPATH . '/www/' );
	} else {
		echo 'Warning: Unable to detect web files. This is normal for database only backups.<br />';
		return true;
	}
	...
	if ( $found_existing ) {
		echo ezimg::genImageTag('bullet_error.png').' Existing WordPress installation detected. Some files may have been overwritten.<br />';
	}
	
	return false;
}

function do_db_import( $zip, $backupdata ) {
	global $legacy;
	if (!$legacy) {
		$file = $_POST['file'];
		...
		if ( file_exists ( ABSPATH . '/wp-content/uploads/temp_'.$zip_id.'/db.sql' ) ) { // full backup
			$file_stream = fopen( ABSPATH . '/wp-content/uploads/temp_'.$zip_id.'/db.sql', "r" );
		} elseif ( file_exists ( ABSPATH . '/db.sql' ) ) { // db only backup
			$file_stream = fopen( ABSPATH . '/db.sql', "r" );
		}
	} else {
		$file_stream = $zip->getStream( 'db/db_0.sql' );
	}
	
	if ( false === $file_stream ) {
		echo 'ERROR: Unable to find any database backup data in the selected backup.<br />';
		return false;
	}
	...
	while ( ! feof( $file_stream ) ) {
		...
		foreach ( (array) $queries as $query ) {
			...
			$result = do_db_import_query( $query, $backupdata );
...
function do_db_import_query( $query, $backupdata ) {
	//echo '<pre>';
	//echo print_r($backupdata);
	//echo '</pre>';
	//echo 'yo'.$query;
	$old_prefix = $backupdata['db_prefix'];
	$new_prefix = mysql_real_escape_string( $_POST['db_prefix'] );
	
	//echo 'old:'.$backupdata['db_prefix'].' new:'.$new_prefix.'<br />';
	
	$query_operators = 'INSERT INTO|CREATE TABLE|REFERENCES|CONSTRAINT';
	
	//echo $query.'<br />';
	// Don't run queries for tables not matching the backed-up site's prefix
	if ( ! preg_match( "/^(?:$query_operators)\s+`?$old_prefix/i", $query ) )
		return false;
	
	
	// This whole ABSPATH section should be moved to after the import.
	// Once the data is imported, it can be fixed using queries.
	
	// Escape windows style paths
	$backup_abs = str_replace( '\\', '\\\\', $backupdata['abspath'] ); // Remove escaping of windows paths.
	$backup_abs = preg_replace( '|/+$|', '', $backup_abs );
	
	$cur_abs = str_replace( '\\', '\\\\', ABSPATH );
	
	// Change absolute file paths.
	$query = str_replace( $backup_abs, $cur_abs, $query );

	// END ABSPATH replace section
	...
	if ( preg_match( "/^CREATE TABLE `([^`]+)`/", $query, $matches ) )
		$result = mysql_query( "DROP TABLE IF EXISTS {$matches[1]}" );
	...
	if (file_exists(ABSPATH.'/wp-config.php')) {
		...
		foreach($lines as $line) {
			if (strstr($line,'DB_NAME')) {
				$wp_config[] = "define('DB_NAME', '".$_POST['db_name']."');\r\n";
			} elseif (strstr($line,'DB_USER')) {
				$wp_config[] = "define('DB_USER', '".$_POST['db_user']."');\r\n";
			} elseif (strstr($line,'DB_PASSWORD')) {
				$wp_config[] = "define('DB_PASSWORD', '".$_POST['db_password']."');\r\n";
			} elseif (strstr($line,'DB_HOST')) {
				$wp_config[] = "define('DB_HOST', '".$_POST['db_server']."');\r\n";
			} elseif (strstr($line,'$table_prefix')) {
				$wp_config[] = '$table_prefix = '."'".$_POST['db_prefix']."';\r\n";
			} else {
				$wp_config[] = $line;
			}
		}
		...
		if ( false === ( $fh = fopen( ABSPATH . '/wp-config.php', 'w' ) ) ) {
			echo 'ERROR: Unable to save changes to wp-config.php.<br />';
			return false;
		}
