            ...
            if (file_exists($path)) {
                ...
                if (is_array($tables) && count($tables) > 0) {
                    foreach ($tables as $table_name) {
                        if (trim($table_name) != '') {
                            $table_name = $table_prefix."GDPT_".$table_name;
                            if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") == $table_name) {
                                $wpdb->query("drop table ".$table_name);
                            }
                        }
            ...
            if (file_exists($path)) {
                ...
                if (is_array($rows) && count($rows) > 0) {
                    foreach ($rows as $row) {
                        if (trim($row) != '') {
                            $a = explode("|", $row);
                            $table = $table_prefix."GDPT_".trim($a[0]);
                            $column = trim($a[1]);
                            ...
                            if (gdDBInstallGDPT::check_column($tbl, $column)) {
